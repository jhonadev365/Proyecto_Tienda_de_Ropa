<?php 
    class Productos extends Controllers{
        ...
        public function settallas(){
            ...
            if($_POST){
                 if( empty($_POST['txtnombre']) ||  empty($_POST['txtprecio']) ||  empty($_POST['txtcantidad'])){
                     $arrresponse= array('status'=>false,'msg'=>'Datos Incorrectos');
                    
                 }else{
 
                     $intidtalla=intval($_POST['idtalla']);
                     $strtalla=strclean($_POST['txtnombre']);
                     $intprecios=intval($_POST['txtprecio']);
                     $intcantidades=intval($_POST['txtcantidad']);
                     $intstatus=intval($_POST['liststatus']);
 
                     if($intidtalla == 0){
                         $requestproducto=$this->model->inserttalla($strtalla,$intprecios,$intcantidades,$intstatus);
                         $option=1;
                    }
                    /*if($intidtalla != 0){
                         $requestproducto=$this->model->updateproducto($intidproducto,$intidcategoria,$intidoferta,$strproducto,$intprecio,$intcantidad,$strfoto,$strdescripcion,$intstatus);
